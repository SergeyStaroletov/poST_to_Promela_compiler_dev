...
public class PromelaModel implements IPromelaElement {
  ...
  public PromelaModel(final Model m, final boolean reduceTimeValues, final boolean addLtlMacrosesToEnd) {
    ...
    final Consumer<Program> _function = (Program p) -> {
      EList<GlobalVarDeclaration> _globVars = m.getGlobVars();
      PromelaProgram _promelaProgram = new PromelaProgram(p, _globVars);
  ...
  private boolean checkProcessesLimit() {
    boolean _xblockexpression = false;
    {
      ...
      int specialProcesses = 0;
      VarSettingProgram _varSettingProgram = PromelaContext.getContext().getVarSettingProgram();
      boolean _notEquals = (!Objects.equal(_varSettingProgram, null));
      if (_notEquals) {
        specialProcesses = PromelaContext.getContext().getVarSettingProgram().getProcessMTypes().size();
      }
