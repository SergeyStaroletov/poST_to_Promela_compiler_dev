/**
 * generated by Xtext 2.26.0
 */
package iae.post.poST;

import org.eclipse.emf.common.util.EList;

import org.eclipse.emf.ecore.EObject;

/**
 * <!-- begin-user-doc -->
 * A representation of the model object '<em><b>Program</b></em>'.
 * <!-- end-user-doc -->
 *
 * <p>
 * The following features are supported:
 * </p>
 * <ul>
 *   <li>{@link iae.post.poST.Program#getName <em>Name</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgInVars <em>Prog In Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgOutVars <em>Prog Out Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgInOutVars <em>Prog In Out Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgVars <em>Prog Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgTempVars <em>Prog Temp Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProgExternVars <em>Prog Extern Vars</em>}</li>
 *   <li>{@link iae.post.poST.Program#getProcesses <em>Processes</em>}</li>
 * </ul>
 *
 * @see iae.post.poST.PoSTPackage#getProgram()
 * @model
 * @generated
 */
public interface Program extends EObject
{
  /**
   * Returns the value of the '<em><b>Name</b></em>' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Name</em>' attribute.
   * @see #setName(String)
   * @see iae.post.poST.PoSTPackage#getProgram_Name()
   * @model
   * @generated
   */
  String getName();

  /**
   * Sets the value of the '{@link iae.post.poST.Program#getName <em>Name</em>}' attribute.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @param value the new value of the '<em>Name</em>' attribute.
   * @see #getName()
   * @generated
   */
  void setName(String value);

  /**
   * Returns the value of the '<em><b>Prog In Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.InputVarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog In Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgInVars()
   * @model containment="true"
   * @generated
   */
  EList<InputVarDeclaration> getProgInVars();

  /**
   * Returns the value of the '<em><b>Prog Out Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.OutputVarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog Out Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgOutVars()
   * @model containment="true"
   * @generated
   */
  EList<OutputVarDeclaration> getProgOutVars();

  /**
   * Returns the value of the '<em><b>Prog In Out Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.InputOutputVarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog In Out Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgInOutVars()
   * @model containment="true"
   * @generated
   */
  EList<InputOutputVarDeclaration> getProgInOutVars();

  /**
   * Returns the value of the '<em><b>Prog Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.VarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgVars()
   * @model containment="true"
   * @generated
   */
  EList<VarDeclaration> getProgVars();

  /**
   * Returns the value of the '<em><b>Prog Temp Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.TempVarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog Temp Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgTempVars()
   * @model containment="true"
   * @generated
   */
  EList<TempVarDeclaration> getProgTempVars();

  /**
   * Returns the value of the '<em><b>Prog Extern Vars</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.ExternalVarDeclaration}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Prog Extern Vars</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_ProgExternVars()
   * @model containment="true"
   * @generated
   */
  EList<ExternalVarDeclaration> getProgExternVars();

  /**
   * Returns the value of the '<em><b>Processes</b></em>' containment reference list.
   * The list contents are of type {@link iae.post.poST.Process}.
   * <!-- begin-user-doc -->
   * <!-- end-user-doc -->
   * @return the value of the '<em>Processes</em>' containment reference list.
   * @see iae.post.poST.PoSTPackage#getProgram_Processes()
   * @model containment="true"
   * @generated
   */
  EList<iae.post.poST.Process> getProcesses();

} // Program
